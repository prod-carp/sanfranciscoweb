...
async function cargarImagenExistenteDesdeR2(
  user,
  imageKey
) {
  ...
  try {
    ...
    const response =
      await fetch(
        `https://sanfrancisco-noticias.produccioncarprinter.workers.dev/api/admin/articles/${encodeURIComponent(
          getEditingArticleId()
        )}/image`,
        {
    ...
    if (
      ...
      throw new Error(
        "No se pudo obtener la imagen desde R2."
      );
...
// ==========================================================
// OBTENER ID DEL ARTÍCULO QUE SE ESTÁ MODIFICANDO
// ==========================================================
